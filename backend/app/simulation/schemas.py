...
class TrainSimulationState(BaseModel):
    ...
    origin_station: Optional[str] = Field(None, description="Route origin station code")
    destination_station: Optional[str] = Field(None, description="Route destination terminus code")
    ...
    has_active_conflict: bool = Field(default=False, description="Whether an active track/signal conflict is detected")
    speed_kmh: float = Field(default=0.0, description="Current operational speed in km/h")
    ai_reasoning: Optional[str] = Field(default=None, description="Dynamic AI operational reasoning generated by simulation backend")
...
class TrainDeltaState(BaseModel):
    ...
    Transmits dynamic telemetry, updated target station schedules, and upcoming stops.
    """
    train_no: int = Field(..., description="Unique train identifier number (e.g. 12003)")
    latitude: float = Field(..., description="Current interpolated geographic latitude")
    longitude: float = Field(..., description="Current interpolated geographic longitude")
    ...
    scheduled_arrival: Optional[str] = Field(None, description="Timetable scheduled arrival time at target stop")
    upcoming_stops: List[Dict[str, Any]] = Field(default_factory=list, description="Next upcoming stations with ETAs")
    has_active_conflict: bool = Field(default=False, description="Whether active conflict is detected")
    ...
    predicted_eta: Optional[str] = Field(None, description="Computed final arrival ETA at next stop")
    speed_kmh: float = Field(default=0.0, description="Current operational speed in km/h")
    ai_reasoning: Optional[str] = Field(default=None, description="Dynamic AI operational reasoning generated by backend")
